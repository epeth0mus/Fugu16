...
func doInstall(pe: PostExploitation, doUpdate: Bool = false) {
    ...
    case .rebootRequired:
        showMessage(withOptions: [
            ...
            kCFUserNotificationAlertMessageKey: "Fugu16 successfully \(doUpdate ? "updated" : "installed") the untether. To complete the \(doUpdate ? "update" : "installation"), your device needs to be rebooted." as NSString,
    ...
    case .ok:
        showSimpleMessage(withTitle: "Snapshot flag not set", andMessage: "Fugu16 or another jailbreak has already been installed. Please restore your RootFS and try again.")
        exit(0)
    
    case .otaAlreadyMounted:
        showSimpleMessage(withTitle: "OTA already mounted", andMessage: "Fugu16 could not be installed because an OTA update is already mounted. Please remove the update, reboot and run the installer again.")
        exit(0)
...
func doUninstall() {
    ...
    case .rebootRequired:
        showMessage(withOptions: [
            kCFUserNotificationAlertTopMostKey: 1 as NSNumber,
            kCFUserNotificationAlertHeaderKey: "Reboot required" as NSString,
            kCFUserNotificationAlertMessageKey: "To complete the uninstallation of Fugu16, your device needs to be rebooted." as NSString,
            kCFUserNotificationDefaultButtonTitleKey: "Reboot now" as NSString
        ])
    ...
    case .noRestoreRequired:
        showSimpleMessage(withTitle: "Not installed", andMessage: "Fugu16 could not be uninstalled because it is not installed.")
        exit(0)
    
    case .failed(reason: let reason):
        showSimpleMessage(withTitle: "Failed to uninstall", andMessage: "Fugu16 could not be uninstalled. Error: \(reason)")
        exit(0)
    }
}
...
// USING Fugu16 WILL NOT CAUSE BOOTLOOPS UNLESS YOU EDIT THE CODE, OK?
func handleBootFailure() -> Never {
    let result = showMessage(withOptions: [
        kCFUserNotificationAlertTopMostKey: 1 as NSNumber,
        kCFUserNotificationAlertHeaderKey: "Fugu16 Untether Safe Mode" as NSString,
...
if action == "untether" {
    if access("/", W_OK) == 0 {
        ...
        if !fastUntetherEnabled {
            run(prog: "/.Fugu16Untether/bin/launchctl", args: ["unload", "/System/Library/LaunchDaemons/com.apple.analyticsd.plist"])
            run(prog: "/.Fugu16Untether/bin/launchctl", args: ["load",   "/Library/LaunchDaemons/com.apple.analyticsd.plist"])
        }
        
        dispatchMain()
    }
    
    if getenv("Fugu16_ERR_CHECK_DONE") == nil {
        // Safety: Check if we failed a few times
        // See the comment above handleBootFailure
        do {
            let counterData = try Data(contentsOf: URL(fileURLWithPath: "/private/var/.Fugu16RebootCounter"))
            ...
            if counter > 4 {
                ...
                try? "1".data(using: .utf8)?.write(to: URL(fileURLWithPath: "/private/var/.Fugu16RebootCounter"))
                handleBootFailure()
            } else if counter < 0 {
                ...
                try? String(counter + 1).data(using: .utf8)?.write(to: URL(fileURLWithPath: "/private/var/.Fugu16RebootCounter"))
        ...
        // Write env vars
        setenv("Fugu16_ERR_CHECK_DONE", "1", 1)
        if errorOccuredDuringLastBoot {
            setenv("Fugu16_PREV_FAILED", "1", 1)
        }
    } else if getenv("Fugu16_PREV_FAILED") != nil {
        errorOccuredDuringLastBoot = true
    }
...
case "untether":
    ...
    pe.unsafelyUnwrapped.injectTC(path: "/.Fugu16Untether/trustcache")
    ...
    if case .ok = pe.unsafelyUnwrapped.untether() {
        do {
            ...
            if let autorun = try? FileManager.default.contentsOfDirectory(atPath: "/.Fugu16Untether/trustcaches/") {
                for exe in autorun {
                    let path = "/.Fugu16Untether/trustcaches/" + exe
            ...
            if let autorun = try? FileManager.default.contentsOfDirectory(atPath: "/.Fugu16Untether/autorun/") {
                for exe in autorun {
                    let path = "/.Fugu16Untether/autorun/" + exe
            ...
            try? "0".data(using: .utf8)?.write(to: URL(fileURLWithPath: "/private/var/.Fugu16RebootCounter"))
            
            // Check if we should show AltStore message
            if access("/.Fugu16Untether/.AltStoreInstall", F_OK) == 0 {
                ...
                unlink("/.Fugu16Untether/.AltStoreInstall")
            }
            
            if errorOccuredDuringLastBoot {
                showSimpleMessage(withTitle: "Fugu16 Untether", andMessage: "The untether failed to exploit the kernel on the previous boot and had to restart the device. This is not an issue and you can ignore this message.")
            }
