...
struct ContentView: View {
    ...
    var body: some View {
        if jailbroken {
            Text("Please reboot into the non-jailbroken state to use the Fugu16 App")
                .padding()
                .alert(isPresented: self.$showJBAlert) {
                    Alert(title: Text("Reboot required"), message: Text("Please reboot into the non-jailbroken state to use the Fugu16 App"), dismissButton: .default(Text("OK")))
                }
        } else if showButton {
            ZStack {
                Button("Setup Fugu16", action: {
                    showButton = false
                    DispatchQueue(label: "Fugu16Setup").async {
                        doSetup()
                    }
                })
                    .padding()
                ...
                VStack {
                    ...
                    Button("Show credits", action: {
                        UIApplication.shared.open(URL(string: "https://github.com/LinusHenze/Fugu16/blob/master/credits.txt")!)
                    })
                        .padding()
                    Button("Show license", action: {
                        UIApplication.shared.open(URL(string: "https://github.com/LinusHenze/Fugu16/blob/master/LICENSE")!)
                    })
                    Spacer()
                }
            }
    ...
    func doSetup() {
        ...
        // Closure Paths
        let spotlightClosurePath     = dyldDir + "/Fugu16App.closure"
