...
let untetherContainerPath   = "/private/var/mobile/Containers/Data/Fugu16Untether"
let untetherClFolder        = "/private/var/Fugu16UntetherDYLD"
...
func installSlowUntether(mountPath: String, trustcache: String, isUpdate: Bool) throws {
    ...
    Logger.print("Installing the Fugu16 untether")
    ...
    try? FileManager.default.createDirectory(atPath: mountPath + "/.Fugu16Untether", withIntermediateDirectories: false, attributes: nil)
    try? FileManager.default.removeItem(atPath: mountPath + "/.Fugu16Untether/stage2")
    try FileManager.default.createSymbolicLink(atPath: mountPath + "/.Fugu16Untether/stage2", withDestinationPath: "/System/Library/CoreServices/ReportCrash")
    try jsUtilsData.write(toFile: mountPath + "/.Fugu16Untether/utils.js", atomically: false, encoding: .utf8)
    try ensureNoDataProtection(mountPath + "/.Fugu16Untether/utils.js")
    try jsSetupData.write(toFile: mountPath + "/.Fugu16Untether/setup.js", atomically: false, encoding: .utf8)
    try ensureNoDataProtection(mountPath + "/.Fugu16Untether/setup.js")
    try jsKernelExploitLauncherData.write(toFile: mountPath + "/.Fugu16Untether/launchKernelExploit.js", atomically: false, encoding: .utf8)
    try ensureNoDataProtection(mountPath + "/.Fugu16Untether/launchKernelExploit.js")
    
    Logger.print("Writing untether binary")
    try? FileManager.default.removeItem(atPath: mountPath + "/.Fugu16Untether/jailbreakd")
    try FileManager.default.copyItem(atPath: exePath, toPath: mountPath + "/.Fugu16Untether/jailbreakd")
    try ensureNoDataProtection(mountPath + "/.Fugu16Untether/jailbreakd")
    
    Logger.print("Writing trust cache")
    try? FileManager.default.removeItem(atPath: mountPath + "/.Fugu16Untether/trustcache")
    try FileManager.default.copyItem(atPath: trustcache, toPath: mountPath + "/.Fugu16Untether/trustcache")
    try ensureNoDataProtection(mountPath + "/.Fugu16Untether/trustcache")
    
    Logger.print("Writing exploit")
    let datAnalytics = try! KeybagClosure(name: "Fugu16Untether").getClosure().emit()
    ...
    let datPs = try! PSClosure(name: "Fugu16Untether-Stage2").getClosure().emit()
    ...
    try? FileManager.default.createDirectory(atPath: mountPath + "/.Fugu16Untether/trustcaches", withIntermediateDirectories: false, attributes: nil)
    try? FileManager.default.createDirectory(atPath: mountPath + "/.Fugu16Untether/autorun", withIntermediateDirectories: false, attributes: nil)
}
...
func installFastUntether(mountPath: String, trustcache: String, isUpdate: Bool) throws {
    ...
    Logger.print("Writing exploit")
    let kbClosure = try! KeybagClosure(name: "Fugu16Untether")
